...
from ui.agent_network_panel import AgentNetworkPanel
...
class MainWindow(QMainWindow):
    ...
    def _create_sidebar(self, parent_layout):
        ...
        self.session_sidebar.agent_network_clicked.connect(self._show_agent_network)
    ...
    def _show_agent_network(self):
        """显示 Agent Network 页面"""
        if not hasattr(self, 'agent_network_page'):
            self.agent_network_page = AgentNetworkPanel()
            self.agent_network_page.back_clicked.connect(self._go_to_chat)
...
开始配置吧！""")
    ...
    def _create_status_bar(self):
        ...
        self.status_bar.showMessage("OpenToad 已就绪")
    ...
    def _save_settings_from_ui(self):
        if hasattr(self, 'settings_widget'):
            new_settings = self.settings_widget.save_settings()
